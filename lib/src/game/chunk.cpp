...
namespace resurgo {
namespace {
constexpr auto noiseScale_v = 0.1f;
} // namespace
...
void Chunk::init(std::uint32_t seed) {
	...
	for (std::size_t y = 0; y < chunkSize_v; y++) {
		for (std::size_t x = 0; x < chunkSize_v; x++) {
			...
			tile.transform.position.z = static_cast<float>(std::floor(n * 5.f)) * tileSize_v;
		}
	}
...
void Chunk::createQuad(glm::vec3 p1, glm::vec3 p2, glm::vec3 p3, glm::vec3 p4) {
	...
	auto v1 = engine::Vertex{.position = p1, .normal = normal, .texCoords = {0, 0}};
	auto v2 = engine::Vertex{.position = p2, .normal = normal, .texCoords = {0, height}};
	auto v3 = engine::Vertex{.position = p3, .normal = normal, .texCoords = {width, height}};
	auto v4 = engine::Vertex{.position = p4, .normal = normal, .texCoords = {width, 0}};
